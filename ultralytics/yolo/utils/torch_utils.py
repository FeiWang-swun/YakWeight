...
def strip_optimizer(f: Union[str, Path] = 'cow-yolov8x-pose.pt', s: str = '') -> None:
    """
    Strip optimizer from 'f' to finalize training, optionally save as 's'.

    Args:
        f (str): file path to model to strip the optimizer from. Default is 'cow-yolov8x-pose.pt'.
    ...
    def __call__(self, epoch, fitness):
        ...
        if stop:
            LOGGER.info(f'Stopping training early as no improvement observed in last {self.patience} epochs. '
                        f'Best results observed at epoch {self.best_epoch}, best model saved as cow-yolov8x-pose.pt.\n'
