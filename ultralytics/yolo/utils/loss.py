...
class KeypointLoss(nn.Module):
    ...
    def forward(self, pred_kpts, gt_kpts, kpt_mask, area):
        ...
        # print("sigmas")
        # print(self.sigmas)
        # sig = np.asarray([0, 0, 0, 0])
        # for idx1 in range(len(gt_kpts)):
        #     for idx2 in range(len(gt_kpts[idx1])):
        #         if gt_kpts[idx1][idx2][0] != 0.0 or gt_kpts[idx1][idx2][1] != 0.0:
        #             sig[idx2] += 1
        # sig = sig*0.25/float(len(gt_kpts))
        # print(sig)
        # sig = torch.tensor(sig).to("cuda")
        # print(sig)
        kpt_loss_factor = (torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0)) / (torch.sum(kpt_mask != 0) + 1e-9)
        # e = d / (2 * (area * self.sigmas) ** 2 + 1e-9)  # from formula
        e = d / (2 * self.sigmas) ** 2 / (area + 1e-9) / 2  # from cocoeval
        # e = d / (2 * sig) ** 2 / (area + 1e-9) / 2  # from cocoeval
